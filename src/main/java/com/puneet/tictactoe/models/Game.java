...
public class Game {
    ...
    public Game(){  //as moves.add() is called, so NPE would not occur.
        this.board = null;
    ...
    public boolean checkDraw(Move move){
        //if all cells are filled => game ends in a draw
    ...
    public void undo(){
        //make-move steps to be reversed here.
        if(moves.size() == 0){
        ...
        //App should go and update the counts hashmaps
        for(WinningStrategy winningStrategy : winningStrategies){
...
/*
    Process flow:
        make move - get who will make move; nextPlayerIndex
        player to be asked,
        Humman - get (row,col), validate
        BOT  -  will move as per difficulty level
